        ...
        private class PassPolicy
        {
            ...
            private bool second()
            {
                if (Password.Length >= Max)
                    return (Password[Max - 1].ToString() == Character);
                else
                    return false;
            }
            ...
            public bool IsValid()
            {
                return (Password[Min - 1].ToString() == Character)
                       ^
                       (
                           second()
                       );
...
3-7 g: gdgtnfggq";
        ...
        private List<PassPolicy> getPolicies(string input)
        {
            var output = new List<PassPolicy>();
            var lines = input?.Split('\n', StringSplitOptions.RemoveEmptyEntries);

            if (lines != null)
                foreach (var line in lines)
                {
                    var policy = new PassPolicy();
                    // 7-8 x: qxrxmxccxxx
                    var data = line.Split(new char[]
                    {
                        ' ', ':', '-'
                    }, StringSplitOptions.RemoveEmptyEntries);
                    policy.Min = int.Parse(data[0]);
                    policy.Max = int.Parse(data[1]);
                    policy.Character = data[2];
                    policy.Password = data[3];
        ...
        public int Answer()
        {
            var policies = getPolicies(GetData());

            if (policies != null) return policies.Count(Index => Index.IsValid());
